...
import SiteThemeModal from "@/components/builder/SiteThemeModal";
...
function BuilderContent() {
  ...
  const [theme, setTheme] = useState({ backgroundColor: "", backgroundImageUrl: "" });
  ...
  useEffect(() => {
    async function load() {
      ...
      setTheme(data.theme || { backgroundColor: "", backgroundImageUrl: "" });
  ...
  const handleAddPage = () => {
    ...
    setPages((prev) => [...prev, { id, name: name.trim(), slug, blocks: [] }]);
    setActivePageId(id);
    setDirty(true);
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateDoc(doc(db, "sites", siteId), { pages, footer, theme });
      setDirty(false);
      setJustSaved(true);
      ...
      <header className="sticky top-0 z-10 border-b border-charcoal-700 bg-charcoal-950/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-6 py-4">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => setShowThemeModal(true)} className="btn-secondary text-sm">
              Theme
            </button>
