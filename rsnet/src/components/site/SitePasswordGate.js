...
export default function SitePasswordGate({ siteId, siteName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // A full reload (rather than router.refresh()) guarantees the
      // server component re-reads the just-set unlock cookie on the very
      // next request — this is what was actually broken before: refresh()
      // could race the cookie write in some browsers/edge configs, making
      // a correct password look like it "didn't unlock" the site.
